...
export function useHardwareSocket(bridgeBaseUrl: string): HardwareSocketStatus {
  const staticDemo = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_STATIC_DEMO) === '1';
  const wsUrl = bridgeBaseUrl.replace(/^http/, 'ws');
  const [status, setStatus] = useState<HardwareSocketStatus>({
    connected: staticDemo,
    port: '',
    simulated: staticDemo,
    mode: 'polling',
    reconnecting: false,
    lastCommandAck: null,
  });
