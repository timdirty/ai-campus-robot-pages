...
def analyze_frame(model, cv2, np, payload: dict) -> dict:
...
def main() -> int:
    try:
        ...
        print(json.dumps({"ready": False, "error": f"missing dependency: {exc}"}, ensure_ascii=False), flush=True)
        return 0
    ...
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"ready": True, "settings": {"imgsz": YOLO_IMAGE_SIZE, "conf": YOLO_CONFIDENCE, "iou": YOLO_IOU, "model": find_model_path()}}, ensure_ascii=False), flush=True)
    except Exception as exc:
    ...
    for line in sys.stdin:
        try:
            ...
            result = analyze_frame(model, cv2, np, payload)
            result["id"] = request_id
            print(json.dumps(result, ensure_ascii=False), flush=True)
        except Exception as exc:
            print(json.dumps({"ok": False, "id": payload.get("id") if "payload" in locals() else None, "error": str(exc)}, ensure_ascii=False), flush=True)
    return 0
